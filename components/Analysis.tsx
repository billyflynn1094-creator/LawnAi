  ...
  return (
    <div className="py-1">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <span className={`w-1 h-1 rounded-full shrink-0 ${tk.faint === 'text-gray-400' ? 'bg-gray-400' : 'bg-field-500'}`} />
          {shopLinks === 'homedepot' ? (
            <a
      ...
      {product.application_rate && (
        <p className={`text-[10px] ${tk.faint} pl-3 mt-0.5`}>Rate: {product.application_rate}</p>
      )}
    </div>
  );
}
    ...
    <div className="flex flex-col gap-3">
      ...
      {primaryProductCount > 0 && (
        <Tile
          ...
          badges={<span className={`text-[10px] ${tk.faint}`}>{primaryProductCount} item{primaryProductCount === 1 ? '' : 's'}</span>}
          tk={tk}
          ...
          {asWellCount > 0 && (
            <div className={`mt-3 pt-3 border-t ${tk.divider}`}>
              <div className="flex items-center gap-1.5 mb-1.5">
                <Badge label="Optional" style={mode === 'light' ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-blue-900/30 text-blue-300 border border-blue-700/30'} />
                <span className={`text-[11px] ${tk.faint} italic`}>
                  For improvement — {asWellGroups.map(g => g.category).filter(Boolean).join(', ')}
                </span>
              </div>
              {asWellGroups.map((g, gi) => (
      ...
      {culturalPractices.length > 0 && (
        <Tile
          icon={<ClipboardList size={16} className={tk.faint} />}
