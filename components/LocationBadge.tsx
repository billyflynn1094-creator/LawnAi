...
/** Single metric tile in the 2x2 grid */
function MetricTile({
  icon,
  label,
  value,
  ...
  diff?: number | null;
  diffUnit?: string;
  /** true = above avg is WARM/WET (amber), false = above avg is wet only (sky) */
  ...
  if (diff != null) {
    ...
    const sign = diff >= 0 ? "+" : "";
    diffEl = (
      <span className={`text-sm font-semibold leading-none ${color}`}>
        {sign}{diff}{diffUnit}
      </span>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-1 rounded-2xl bg-soil-800/70 border border-white/6 px-4 py-3">
      <div className="flex items-center gap-1.5 text-field-400">
        {icon}
        <span className="text-xs font-medium text-field-400 uppercase tracking-wide">{label}</span>
      </div>
      <span className="text-2xl font-bold text-white leading-tight">{value}</span>
      {diffEl ?? <span className="text-sm text-field-600">no baseline yet</span>}
    </div>
  );
}
...
export default function LocationBadge({
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-soil-800 text-field-300">
  ...
  if (error) {
    return (
      <button
        onClick={onRetry}
        className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-rust-500/20 text-rust-300 hover:bg-rust-500/30 transition w-full"
      >
  ...
  const airValue = weather
    ? `${weather.avg_high_f}° / ${weather.avg_low_f}°`
    : "—";
  ...
  return (
    <div className="rounded-2xl bg-soil-900/70 border border-white/8 overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 px-4 pt-3 pb-2">
        {location.city && (
          <div className="flex items-center gap-1.5">
            <MapPin size={12} className="text-field-500 shrink-0" />
        ...
        {location.hardiness_zone && location.hardiness_zone !== "Unknown" && (
          <span className="text-xs text-field-400 bg-soil-800 rounded-full px-2 py-0.5">
            Zone {location.hardiness_zone}
          </span>
        )}
        {grassLabel && (
          <span className="flex items-center gap-1 text-xs text-field-400 bg-soil-800 rounded-full px-2 py-0.5">
            <Leaf size={10} className="shrink-0" />
            {grassLabel}
          </span>
        )}
        {location.soilType && (
          <span className="text-xs text-field-500 truncate max-w-[180px]">
      ...
      {/* ── Data methodology tag ── */}
      <div className="flex items-center justify-center gap-1 pb-2.5">
        <Navigation size={9} className="text-field-600" />
        <span className="text-[10px] text-field-600 font-medium tracking-widest uppercase">
