...
import VideoScan from "@/components/irrigation/VideoScan";
...
const SCREEN_LABELS: Record<Screen, string> = {
  ...
  design: "Design Scan", controller: "Controller Guide", zones: "Zone Assessment",
  audit_report: "Audit Report", video: "Video Scan",
};
...
export default function IrrigationPage() {
      ...
      reader.onload = async (ev) => {
        ...
        try {
          ...
          if (a && !a.error) {
            ...
            setSmartResult({ brief: "Couldn't analyze this image", detail: data.error ?? "Try better lighting, get closer, or upload from gallery.", action: "Try again or choose a path below", severity: "moderate" });
          }
        } catch (err) {
          setSmartResult({ brief: "Scan failed — " + (err instanceof Error ? err.message : "Network error"), detail: "Check your connection or upload a saved photo.", severity: "moderate" });
  ...
  const DIAG_ROUTES: Record<string, { screen: Screen; preset?: string }> = {
    electrical: { screen: "electrical" }, hydraulic: { screen: "hydraulic" },
    rain_sensor: { screen: "rain_sensor" }, design: { screen: "design" },
    controller: { screen: "controller" }, zones: { screen: "zones" },
  };
      ...
      <main className="px-4 max-w-lg mx-auto pb-10">
        ...
        <div className="space-y-2.5">
          <button onClick={() => nav("video")}
            className="w-full flex items-center gap-4 p-4 rounded-xl border border-blue-600/40 bg-blue-900/10 hover:bg-blue-900/18 text-left transition-all">
            <span className="text-2xl">🎥</span>
            <div className="flex-1">
              <p className="text-white text-sm font-semibold">Video scan</p>
              <p className="text-blue-300/60 text-xs">Live camera stream • Upload video • Glasses-ready</p>
            </div>
            <span className="text-xs text-blue-500 border border-blue-500/30 px-2 py-0.5 rounded-full flex-shrink-0">NEW</span>
          </button>
          <button onClick={() => setScreen("broken")}
            ...
            <span className="text-2xl">🔴</span>
            <div><p className="text-white text-sm font-semibold">Something's broken</p><p className="text-red-300/60 text-xs">Pick the symptom, run diagnosis</p></div>
          </button>
          <button onClick={() => { setAuditStep(0); setScreen("audit"); }}
            ...
            <span className="text-2xl">📋</span>
            <div><p className="text-white text-sm font-semibold">Full inspection</p><p className="text-blue-300/60 text-xs">Systematic camera walkthrough</p></div>
          </button>
          <button onClick={() => nav("controller")}
            ...
            <div><p className="text-white text-sm font-medium">Controller Guide</p><p className="text-gray-500 text-xs">Program any controller, step by step</p></div>
          </button>
          {seniorMode && (
            <button onClick={() => nav("electrical", "two_wire")}
              ...
              <span className="text-2xl">⚡</span>
              <div><p className="text-purple-200 text-sm font-medium">Two-Wire Decoder Diag</p><p className="text-purple-400/60 text-xs">Senior tech mode</p></div>
            </button>
          )}
        </div>
      </main>
      ...
      <main className="px-4 max-w-lg mx-auto pb-8">
        ...
        <div className="space-y-2.5">
          {AUDIT_STEPS.map((step, i) => {
            ...
            return (
              <button key={step.id} onClick={() => { if (status !== "upcoming") nav(step.screen); }} disabled={status === "upcoming"}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#05111f] to-[#0a1f35]">
      <header className="sticky top-12 z-10 bg-[#05111f]/90 backdrop-blur-md border-b border-white/5 px-4 py-3">
        <div className="max-w-lg mx-auto flex items-center gap-3">
          <button onClick={back} className="text-blue-400 hover:text-blue-300 transition-colors"><ArrowLeft className="w-5 h-5" /></button>
